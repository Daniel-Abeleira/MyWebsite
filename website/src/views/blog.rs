...
fn get_posts_by_year(year: i32, exclude_id: &str) -> Vec<(String, BlogPost)> {
    load_all_posts()
        .into_iter()
        .filter(|(post_id, post)| post.publication_year == year && post_id != exclude_id)
        .collect()
}
...
pub fn Blog(id: String) -> Element {
    if let Some(post) = get_post_by_id(&id) {
        let other_posts = get_posts_by_year(post.publication_year, &id);
        ...
        rsx! {
            ...
            hr {}
            section {
                h2 { "Other posts from {post.publication_year}:" }
                ul {
                    {
